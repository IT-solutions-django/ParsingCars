...
async def request_photos(images):
    filtered_photos = list(filter(lambda s: s.strip(), images))
    filtered_photos = [f"http://www.autohub.co.kr{s}" for s in filtered_photos]

    semaphore = asyncio.Semaphore(50)

    async with aiohttp.ClientSession() as session:
        tasks = [check_photo_url(session, url, semaphore) for url in filtered_photos]
